...
@Component
@Order(3)
public class AuctionBidderNotSellerValidator implements BidValidationStrategy {
    @Override
    public void validate(Auction auction, UUID bidderId, BigDecimal amount) {
        if (auction.getSellerId().equals(bidderId)) {
            throw new BidValidationException("Seller cannot bid on their own auction", "BIDDER_IS_SELLER");
        }
    }
}
